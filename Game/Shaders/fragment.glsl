...
void main()
{
    ...
    float VdotR =  dot(view_direction, reflect(light.direction, world_normal));


    vec3 fresnel = SchlickFresnel(texture(material.diffuse, interpolated_texture_coordinate).rgb, NdotL);
    float shininess = 1.5;
    float attenuation = 1.0;
    vec3 step1 = (texture(material.diffuse, interpolated_texture_coordinate).rgb * (vec3(1.0) - texture(material.specular, interpolated_texture_coordinate).rgb)) / PI;
    vec3 step2 = ((shininess + 2.0) / (2.0 * PI)) * fresnel * pow(VdotR, shininess);
    vec3 PBR = light.diffuse * (step1 + step2) * NdotL * attenuation;
